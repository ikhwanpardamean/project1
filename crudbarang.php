...
class Crudbarang
{
    ...
    public function add_data($id, $kode_barang, $nama_barang, $satuan, $id_jenis, $id_supplier, $harga, $stok)
    {
        $data = $this->db->prepare('INSERT INTO barang (id,kode_barang,nama_barang,satuan,id_jenis,id_supplier,harga,stok) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        
        $data->bindParam(1, $id);
        $data->bindParam(2, $kode_barang);
        $data->bindParam(3, $nama_barang);
        $data->bindParam(4, $satuan);
        $data->bindParam(5, $id_jenis);
        $data->bindParam(6, $id_supplier);
        $data->bindParam(7, $harga);
        $data->bindParam(8, $stok);
